...
class Samples(object):
    ...
    def get_images(self, connection):
        ...
        for im in connection.execute(
                _GET_IMAGE,
                project_name=self._capture_zone.project_name,
...
class ImageSource(object):
    ...
    def get_images(self):
        for im in self._images:
            yield im.label, self._filtering_model.filter_image(im.image)
...
class SamplingView(object):
    ...
    def render(self, container):
        ...
        self._label_instance = li = tk.Label(lf, text="Instance")
...
class SamplingController(object):
    ...
    def save(self):
        image = self._image
        if image:
            self._samples.add_sample(image, self._label)
    ...
    def set_label(self, *args):
        # todo: notify label observers
        if self._thumbnail_set:
            lc = self._label_class
            view = self._sampling_view
            cz = self._capture_zone

            samples = next((l for l in self._labels[self._label_type] if l.label_name == lc))

            #todo: we need to set the detection to the samples
            #todo: detect should be defined in samples

            if samples.classifiable:
                view.menu.entryconfig("Save", state=tk.ACTIVE)
                cz.set_detection(mdl.DifferenceMatching(self._image_source))
            else:
                view.menu.entryconfig("Save", state=tk.DISABLED)
                cz.set_detection(mdl.Tesseract())
    ...
    def detect(self):
        detected = self._capture_zone.detect(
            self._filtering_model.filter_image(self._image))
